...
const sortFiles = (arr) => arr.sort((a, b) => a.localeCompare(b, undefined, { numeric: true, sensitivity: 'base' }));
...
(async () => {
    const pattern = toPosix(path.join(IMAGES_ROOT, '**/*.{png,PNG,webp,WEBP,jpg,JPG,jpeg,JPEG,svg,SVG,avif,AVIF}'));
    ...
    const result = await new Promise((resolve, reject) =>
        Spritesmith.run({ src: normalizedPngs, algorithm: 'binary-tree', padding: 4 }, (err, res) =>
...
`;
    ...
    console.log(`Sprite built: ${OUT_WEBP} (${SPRITE_W}x${SPRITE_H}) with ${items.length} items → ${OUT_TS}`);
